...
import GroupCategoryForm from './group_category_form';

/**
 * User interface for listing, filtering and exporting Group Categories. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model. 
 */
class GroupCategories extends React.Component {
  static propTypes = {
    /** @type {object} A hash of group_category ids to objects containing their attributes and associated models' attributes. */
    group_categories: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      group_categories: this.copy(props.group_categories),
   ...
   * @param {object} group_category - The model instance to add to the current state.
   * @public
   */
  add(group_category) {
    this.setState(function(prevState, props){
      prevState.group_categories[group_category.id] = group_category;
   ...
   * @param {object} group_category - The updated model instance replace in the current state.
   * @public
   */
   ...
   * @param {object} group_category - The deleted model instance remove from the current state.
   * @public
   */
  delete(group_category){
    this.setState(function(prevState, props){
      delete prevState.group_categories[group_category.id];
      prevState.selected = null;
      prevState.editing = false;
      return prevState;
    });
  ...
  render(){
    ...
    if (this.state.adding){
      ...
      top_content = <GroupCategoryForm 
        action="update" 
        group_category={this.state.selected} 
        current_user={this.props.current_user}
    ...
    return (
      <div className="group_categories col-md-12">
        ...
        <div className="card">
          ...
          <h4 className="text-center">
            (Filtered to {this.state.export_data.length} out of {Object.keys(this.state.group_categories).length})
          </h4>
          ...
          <div className="card-body">
            ...
            <CSVLink 
              ...
              filename="group_categories_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.group_categories)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.group_categories = this.copy(nextProps.group_categories);
      return prevState;
    })
  }
}
