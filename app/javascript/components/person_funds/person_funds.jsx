...
import PersonFund from './person_fund';
import PersonFundForm from './person_fund_form';

/**
 * User interface for listing, filtering and exporting Person Funds. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class PersonFunds extends React.Component {
  static propTypes = {
    /** @type {object} A hash of person_fund ids to objects containing their attributes and associated models' attributes. */
    person_funds: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      person_funds: this.copy(props.person_funds),
   ...
   * @param {object} person_fund - The model instance to add to the current state.
   * @public
   */
  add(person_fund) {
    this.setState(function(prevState, props){
      prevState.person_funds[person_fund.id] = person_fund;
   ...
   * @param {object} person_fund - The updated model instance replace in the current state.
   * @public
   */
  update(person_fund){
    this.setState(function(prevState, props){
   ...
   * @param {object} person_fund - The deleted model instance remove from the current state.
   * @public
   */
  delete(person_fund){
    this.setState(function(prevState, props){
      delete prevState.person_funds[person_fund.id];
      prevState.selected = null;
      prevState.editing = false;
      return prevState;
    });
  ...
  render(){
    ...
    if (this.state.adding){
      ...
      top_content = <PersonFundForm
        action="update"
        person_fund={this.state.selected}
        current_user={this.props.current_user}
    ...
    return (
      <div className="person_funds col-md-12 p-0">
        ...
        <div className="card">
          <h2 className="card-title text-center mt-3">
            {this.props.parent && this.props.parent.name ? this.props.parent.name : null} Money Given
          </h2>

          <h4 className="text-center">
            (Filtered to {this.state.export_data.length} out of {Object.keys(this.state.person_funds).length})
          </h4>
          ...
          <div className="card-body">
            ...
            <CSVLink
              ...
              filename="person_funds_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.person_funds)}
              minRows={1}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.person_funds = this.copy(nextProps.person_funds);
      return prevState;
    })
  }
}

export default PersonFunds;
