...
import ConsultationCategory from './consultation_category';
import ConsultationCategoryForm from './consultation_category_form';

/**
 * User interface for listing, filtering and exporting Consultation Categories. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class ConsultationCategories extends React.Component {
  static propTypes = {
    /** @type {object} A hash of consultation_category ids to objects containing their attributes and associated models' attributes. */
    consultation_categories: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      consultation_categories: this.copy(props.consultation_categories),
   ...
   * @param {object} consultation_category - The model instance to add to the current state.
   * @public
   */
  add(consultation_category) {
    this.setState(function(prevState, props){
      prevState.consultation_categories[consultation_category.id] = consultation_category;
      prevState.adding = false;

      prevState.editing = true;
      prevState.selected = consultation_category;

      prevState.status_message = "consultation_category"+consultation_category.id+" created.";
   ...
   * @param {object} consultation_category - The updated model instance replace in the current state.
   * @public
   */
  update(consultation_category){
    this.setState(function(prevState, props){
      prevState.consultation_categories[consultation_category.id] = consultation_category;

      prevState.status_message = "consultation_category"+consultation_category.id+" updated.";
      prevState.status_message_type = "success";
   ...
   * @param {object} consultation_category - The deleted model instance remove from the current state.
   * @public
   */
  delete(consultation_category){
    this.setState(function(prevState, props){
      delete prevState.consultation_categories[consultation_category.id];
      prevState.selected = null;
      prevState.editing = false;

      prevState.status_message = "consultation_category"+consultation_category.id+" deleted.";
      prevState.status_message_type = "success";
  ...
  render(){
    ...
    if (this.state.adding){
      top_content = <ConsultationCategoryForm
        action="create"
      ...
      top_content = <ConsultationCategoryForm
        action="update"
        consultation_category={this.state.selected}
        current_user={this.props.current_user}
        parent_model={this.props.parent_model}
      ...
      top_content = <ConsultationCategory
        consultation_category={this.state.selected}
        current_user={this.props.current_user}
    ...
    return (
      <div className="consultation_categories col-md-12">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <CSVLink
              ...
              filename="consultation_categories_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.consultation_categories)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.consultation_categories = this.copy(nextProps.consultation_categories);
      return prevState;
    })
  }
}
