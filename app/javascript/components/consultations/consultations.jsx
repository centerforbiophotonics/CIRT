...
import ConsultationForm from './consultation_form';

/**
 * User interface for listing, filtering and exporting Consultations. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class Consultations extends React.Component {
  static propTypes = {
    /** @type {object} A hash of consultation ids to objects containing their attributes and associated models' attributes. */
  ...
  constructor(props){
    ...
    this.state = {
      ...
      consultations: this.copy(props.consultations),
   ...
   * @param {object} consultation - The model instance to add to the current state.
   * @public
   */
  add(consultation) {
    this.setState(function(prevState, props){
      prevState.consultations[consultation.id] = consultation;
      prevState.adding = false;

      prevState.editing = true;
      prevState.selected = consultation;

      prevState.status_message = "consultation"+consultation.id+" created.";
   ...
   * @param {object} consultation - The updated model instance replace in the current state.
   * @public
   */
  update(consultation){
    this.setState(function(prevState, props){
      ...
      prevState.status_message = "consultation"+consultation.id+" updated.";
      prevState.status_message_type = "success";
   ...
   * @param {object} consultation - The deleted model instance remove from the current state.
   * @public
   */
  delete(consultation){
    this.setState(function(prevState, props){
      delete prevState.consultations[consultation.id];
      prevState.selected = null;
      prevState.editing = false;

      prevState.status_message = "consultation"+consultation.id+" deleted.";
      prevState.status_message_type = "success";
  ...
  columnDefs(){
    return [
      ...
      { Header: 'Consultants', accessor: 'consultants' },
      { Header: 'Minutes', accessor: 'minutes' },
      { Header: 'Consultation Category', accessor: 'consultation_category_id' },
      { Header: 'CCRC', accessor: 'ccrc_id' },
      {
        Header: 'Actions',
        Cell: d => {
          return (
  ...
  render(){
    ...
    if (this.state.adding){
      ...
      top_content = <ConsultationForm
        action="update"
        consultation={this.state.selected}
        current_user={this.props.current_user}
        parent_model={this.props.parent_model}
    ...
    return (
      <div className="consultations col-md-12">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <ReactTable
              data={Object.values(this.state.consultations)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.consultations = this.copy(nextProps.consultations);
      return prevState;
    })
  }
}
