...
import PersonScantronAppointment from './person_scantron_appointment';
import PersonScantronAppointmentForm from './person_scantron_appointment_form';

/**
 * User interface for listing, filtering and exporting Person Scantron Appointments. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class PersonScantronAppointments extends React.Component {
  static propTypes = {
    /** @type {object} A hash of person_scantron_appointment ids to objects containing their attributes and associated models' attributes. */
    person_scantron_appointments: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      person_scantron_appointments: this.copy(props.person_scantron_appointments),
   ...
   * @param {object} person_scantron_appointment - The model instance to add to the current state.
   * @public
   */
  add(person_scantron_appointment) {
    this.setState(function(prevState, props){
      prevState.person_scantron_appointments[person_scantron_appointment.id] = person_scantron_appointment;
      prevState.adding = false;

      prevState.editing = true;
      prevState.selected = person_scantron_appointment;

      prevState.status_message = "person_scantron_appointment"+person_scantron_appointment.id+" created.";
   ...
   * @param {object} person_scantron_appointment - The updated model instance replace in the current state.
   * @public
   */
  update(person_scantron_appointment){
    this.setState(function(prevState, props){
      prevState.person_scantron_appointments[person_scantron_appointment.id] = person_scantron_appointment;

      prevState.status_message = "person_scantron_appointment"+person_scantron_appointment.id+" updated.";
      prevState.status_message_type = "success";
   ...
   * @param {object} person_scantron_appointment - The deleted model instance remove from the current state.
   * @public
   */
  delete(person_scantron_appointment){
    this.setState(function(prevState, props){
      delete prevState.person_scantron_appointments[person_scantron_appointment.id];
      prevState.selected = null;
      prevState.editing = false;

      prevState.status_message = "person_scantron_appointment"+person_scantron_appointment.id+" deleted.";
      prevState.status_message_type = "success";
  ...
  columnDefs(){
    return [
      { Header: 'Person', accessor: 'person_id' },
      { Header: 'Scantron Appointment', accessor: 'scantron_appointment_id' },
      {
        Header: 'Actions',
        Cell: d => {
          return (
  ...
  render(){
    ...
    if (this.state.adding){
      top_content = <PersonScantronAppointmentForm
        action="create"
      ...
      top_content = <PersonScantronAppointmentForm
        action="update"
        person_scantron_appointment={this.state.selected}
        current_user={this.props.current_user}
        parent_model={this.props.parent_model}
      ...
      top_content = <PersonScantronAppointment
        person_scantron_appointment={this.state.selected}
        current_user={this.props.current_user}
    ...
    return (
      <div className="person_scantron_appointments col-md-12 p-0">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <CSVLink
              ...
              filename="person_scantron_appointments_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.person_scantron_appointments)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.person_scantron_appointments = this.copy(nextProps.person_scantron_appointments);
      return prevState;
    })
  }
}

export default PersonScantronAppointments;
