...
import ScantronAppointment from './scantron_appointment';
import ScantronAppointmentForm from './scantron_appointment_form';

/**
 * User interface for listing, filtering and exporting Scantron Appointments. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class ScantronAppointments extends React.Component {
  static propTypes = {
    /** @type {object} A hash of scantron_appointment ids to objects containing their attributes and associated models' attributes. */
    scantron_appointments: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      scantron_appointments: this.copy(props.scantron_appointments),
   ...
   * @param {object} scantron_appointment - The model instance to add to the current state.
   * @public
   */
  add(scantron_appointment) {
    this.setState(function(prevState, props){
      prevState.scantron_appointments[scantron_appointment.id] = scantron_appointment;
      prevState.adding = false;

      prevState.editing = true;
      prevState.selected = scantron_appointment;

      prevState.status_message = "scantron_appointment"+scantron_appointment.id+" created.";
   ...
   * @param {object} scantron_appointment - The updated model instance replace in the current state.
   * @public
   */
  update(scantron_appointment){
    this.setState(function(prevState, props){
      prevState.scantron_appointments[scantron_appointment.id] = scantron_appointment;

      prevState.status_message = "scantron_appointment"+scantron_appointment.id+" updated.";
      prevState.status_message_type = "success";
   ...
   * @param {object} scantron_appointment - The deleted model instance remove from the current state.
   * @public
   */
  delete(scantron_appointment){
    this.setState(function(prevState, props){
      delete prevState.scantron_appointments[scantron_appointment.id];
      prevState.selected = null;
      prevState.editing = false;

      prevState.status_message = "scantron_appointment"+scantron_appointment.id+" deleted.";
      prevState.status_message_type = "success";
  ...
  columnDefs(){
    return [
      ...
      { Header: 'Term Code', accessor: 'term_code' },
      { Header: 'Number Of Scantrons', accessor: 'number_of_scantrons' },
      {
        Header: 'Actions',
        Cell: d => {
          return (
  ...
  render(){
    ...
    if (this.state.adding){
      top_content = <ScantronAppointmentForm
        action="create"
      ...
      top_content = <ScantronAppointmentForm
        action="update"
        scantron_appointment={this.state.selected}
        current_user={this.props.current_user}
        parent_model={this.props.parent_model}
      ...
      top_content = <ScantronAppointment
        scantron_appointment={this.state.selected}
        current_user={this.props.current_user}
    ...
    return (
      <div className="scantron_appointments col-md-12">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <CSVLink
              ...
              filename="scantron_appointments_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.scantron_appointments)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.scantron_appointments = this.copy(nextProps.scantron_appointments);
      return prevState;
    })
  }
}

export default ScantronAppointments;
