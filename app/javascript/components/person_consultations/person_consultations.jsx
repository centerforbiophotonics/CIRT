...
import PersonConsultation from './person_consultation';
import PersonConsultationForm from './person_consultation_form';

/**
 * User interface for listing, filtering and exporting Person Consultations. Renders subcomponents for CRUD actions. Can be used a root component or rendered from the form or show component of an associated model.
 */
class PersonConsultations extends React.Component {
  static propTypes = {
    /** @type {object} A hash of person_consultation ids to objects containing their attributes and associated models' attributes. */
    person_consultations: PropTypes.object,
  ...
  constructor(props){
    ...
    this.state = {
      ...
      person_consultations: this.copy(props.person_consultations),
   ...
   * @param {object} person_consultation - The model instance to add to the current state.
   * @public
   */
  add(person_consultation) {
    this.setState(function(prevState, props){
      prevState.person_consultations[person_consultation.id] = person_consultation;
      prevState.adding = false;

      prevState.editing = true;
      prevState.selected = person_consultation;

      prevState.status_message = "person_consultation"+person_consultation.id+" created.";
   ...
   * @param {object} person_consultation - The updated model instance replace in the current state.
   * @public
   */
  update(person_consultation){
    this.setState(function(prevState, props){
      prevState.person_consultations[person_consultation.id] = person_consultation;

      prevState.status_message = "person_consultation"+person_consultation.id+" updated.";
      prevState.status_message_type = "success";
   ...
   * @param {object} person_consultation - The deleted model instance remove from the current state.
   * @public
   */
  delete(person_consultation){
    this.setState(function(prevState, props){
      delete prevState.person_consultations[person_consultation.id];
      prevState.selected = null;
      prevState.editing = false;

      prevState.status_message = "person_consultation"+person_consultation.id+" deleted.";
      prevState.status_message_type = "success";
  ...
  columnDefs(){
    return [
      {
        Header: 'Person',
        accessor: d => (d.person == undefined ?
          this.props.parent_model.name
          :
          d.person.name
        ),
        id: 'personName' },
      { Header: 'Consultation Date', accessor: d => d.consultation.dates.join(", "), id: 'consultationDate' },
      { Header: 'Consultants', accessor: d => d.consultation.consultants.join(", "), id: 'consultationConsultants' },
      { Header: 'Consultation Category', accessor: d => d.consultation.consultation_category.name, id: 'consultationCategoryName' },
      {
        Header: 'Actions',
        Cell: d => {
          return (
  ...
  render(){
    ...
    if (this.state.adding){
      top_content = <PersonConsultationForm
        action="create"
      ...
      top_content = <PersonConsultationForm
        action="update"
        person_consultation={this.state.selected}
        current_user={this.props.current_user}
        parent_model={this.props.parent_model}
      ...
      top_content = <PersonConsultation
        person_consultation={this.state.selected}
        current_user={this.props.current_user}
    ...
    return (
      <div className="person_consultations col-md-12 p-0">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <CSVLink
              ...
              filename="person_consultations_export.csv"
            >
              <FontAwesomeIcon icon="file-export"/>
            </CSVLink>

            <ReactTable
              data={Object.values(this.state.person_consultations)}
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(prevState => {
      prevState.person_consultations = this.copy(nextProps.person_consultations);
      return prevState;
    })
  }
}

export default PersonConsultations;
